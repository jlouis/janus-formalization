...
Section Janus1.
  ...
  Section Stmt.
    ...
    | se_assvar_xor: forall (d : defs) (m m': mem) (v: Var) (n n' n'': w32) (e: Exp),
    ...
    Proof.
      ...
      inversion H. inversion H4. subst.
        assert (Stm_eval d m'1 (S_Semi s2 s3) m'). constructor 5 with (m' := m'0);
          assumption. constructor 5 with (m' := m'1); assumption.
      inversion H. subst. inversion H6. subst.
        assert (Stm_eval d m (S_Semi s1 s2) m'1). constructor 5 with (m' := m'0);
          assumption.
   ...
   Proof.
      ...
      inversion H4. subst.
      assert (W32Mem.hide m v = W32Mem.hide m'' v). eapply W32Mem.write_hide. eauto.
      assert (Word32.xor n n' = Word32.xor n0 n'0). assert (W32Mem.write m v (Word32.xor n n') v = W32Mem.write m'' v (Word32.xor n0 n'0) v).
      apply equal_f. trivial. eapply W32Mem.write_eq_2. eauto.
      assert (n = n0). assert (Some n = Some n0). rewrite H2 in H0. congruence. grind.
      subst.
      assert (n'0 = n'). eapply Word32.xor_mine. eauto.
      subst. eapply (W32Mem.hide_eq m m'' v); eauto.
  ...
  Section Invert.
    ...
    Proof.
      ...
      simpl. apply (se_assvar_xor d m' m v n n'' n').
        ...
        rewrite <- H1. rewrite <- Word32.xor_assoc. rewrite Word32.xor_x_x_zero.
        rewrite Word32.xor_commut. rewrite Word32.xor_zero. trivial.
        symmetry. trivial.
      apply (W32Mem.hide_eq m (W32Mem.write m' v n') v n'). trivial.
        rewrite W32Mem.write_eq. trivial. rewrite W32Mem.hide_write. rewrite H2.
        rewrite W32Mem.hide_write. trivial.
      ...
      apply (se_assvar_xor G m0 m'0 v n n'' n').
        rewrite H8. rewrite W32Mem.hide_write. assumption.
        rewrite H8. apply W32Mem.write_eq.
        assert (n' = Word32.xor n n'').
        rewrite <- H6. rewrite <- Word32.xor_assoc. rewrite Word32.xor_x_x_zero.
        rewrite Word32.xor_commut. rewrite Word32.xor_zero. trivial.
        symmetry. trivial. 
        apply (W32Mem.hide_eq m'0 (W32Mem.write m0 v n') v n'). trivial. apply W32Mem.write_eq.
        rewrite W32Mem.hide_write. rewrite H8. rewrite W32Mem.hide_write. trivial.
      intros.
